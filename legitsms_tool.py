#!/usr/bin/env python3
"""
LegitSMS desktop helper.

Features:
- Input API key
- Buy number via getNumber
- Auto refresh getStatus every 3 seconds
- Show latest SMS code/result
- Cancel/complete order via setStatus
"""
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.api_key_entry = ttk.Entry(top, width=60, show="*")
        ...
        self.service_entry = ttk.Entry(top, width=25)
        self.service_entry.insert(0, "wa")
        self.service_entry.grid(row=2, column=1, sticky="w", pady=4)
        ...
        ttk.Button(actions, text="Check SMS Now", command=self.check_status_once).pack(side="left", padx=(0, 8))
        ttk.Button(actions, text="Start Auto Refresh (3s)", command=self.start_polling).pack(side="left", padx=(0, 8))
        ttk.Button(actions, text="Stop Refresh", command=self.stop_polling).pack(side="left", padx=(0, 8))
        ...
        self.log("Ready. Set API key, then click Get Number.")
    ...
    def start_polling(self):
        ...
        if self.polling:
            return
        self.polling = True
        self.log("Auto refresh started: every 3 seconds.")
        self._poll_tick()
